...
namespace cuda {
  ...
  __host__ __device__ uint32_t getLink(uint32_t ww) {
    return ((ww >> pixelgpudetails::LINK_shift) & pixelgpudetails::LINK_mask);
  }

  __host__ __device__ uint32_t getRoc(uint32_t ww) {
    return ((ww >> pixelgpudetails::ROC_shift) & pixelgpudetails::ROC_mask);
  }

  __host__ __device__ uint32_t getADC(uint32_t ww) {
    return ((ww >> pixelgpudetails::ADC_shift) & pixelgpudetails::ADC_mask);
  }

  __host__ __device__ bool isBarrel(uint32_t rawId) { return (1 == ((rawId >> 25) & 0x7)); }

  __host__ __device__ bool rocRowColIsValid(uint32_t rocRow, uint32_t rocCol) {
  ...
  __host__ __device__ bool dcolIsValid(uint32_t dcol, uint32_t pxid) {
    return ((dcol < 26) & (2 <= pxid) & (pxid < 162));
  }

  __host__ __device__ pixelgpudetails::DetIdGPU getRawId(const SiPixelFedCablingMapGPU* cablingMap,
                                                         uint8_t fed,
                                                         uint32_t link,
                                                         uint32_t roc) {
    uint32_t index =
  ...
  __host__ __device__ pixelgpudetails::Pixel frameConversion(
  ...
  __host__ __device__ uint8_t conversionError(uint8_t fedId, uint8_t status, bool debug = false) {
    // debug = true;
  ...
  __host__ __device__ uint8_t checkROC(
