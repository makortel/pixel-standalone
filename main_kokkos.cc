...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < NLOOPS; ++i) {
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<Kokkos::Cuda>(0, input.wordCounter),
        KOKKOS_LAMBDA(const size_t i) { kokkos::rawtodigi(input_d, output_d, wordCounter, true, true, false, i); });
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<
            ...
            >(0, input.wordCounter),
        KOKKOS_LAMBDA(const size_t i) { kokkos::rawtodigi(input_h, output_h, wordCounter, true, true, false, i); });
    Kokkos::fence();
    auto stop = std::chrono::high_resolution_clock::now();
