...
namespace kokkos_serial {
  ...
  KOKKOS_INLINE_FUNCTION
  constexpr Packing packing() { return Packing(11, 11, 0, 10); }
  ...
  KOKKOS_INLINE_FUNCTION
  pixelgpudetails::DetIdGPU getRawId(const SiPixelFedCablingMapKokkosDevice<MemorySpace>& cablingMap,
                                     uint8_t fed,
                                     uint32_t link,
                                     uint32_t roc) {
    uint32_t index =
  ...
  KOKKOS_INLINE_FUNCTION
  uint8_t checkROC(
      uint32_t errorWord, uint8_t fedId, uint32_t link, const SiPixelFedCablingMapKokkosDevice<MemorySpace>& cablingMap, bool debug = false) {
  ...
  template <typename MemorySpace>
  KOKKOS_FUNCTION void rawtodigi(const InputKokkos<MemorySpace>& input,
                                 Output* output,
    ...
    for (int32_t iloop = first, nend = index + 1; iloop < nend; ++iloop) {
      ...
      auto fedIds = input.GetDeviceFedId();
      ...
      auto cablingMap_d = input.GetDeviceCablingMap();

      auto words = input.GetDeviceWord();
